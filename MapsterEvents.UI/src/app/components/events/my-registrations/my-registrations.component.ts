...
@Component({
  ...
  template: `
    <div class="container">
      <div class="row">
        <div class="col-12">
          <h2 class="mb-4">Kayıtlı Olduğum Etkinlikler</h2>
          ...
          <div *ngIf="events.length === 0" class="text-center py-5">
            <p class="text-muted">Henüz kayıtlı olduğunuz etkinlik bulunmamaktadır.</p>
            <a routerLink="/events" class="btn btn-primary">Etkinlikleri Keşfet</a>
          </div>
        </div>
      </div>
    </div>
...
export class MyRegistrationsComponent implements OnInit {
  ...
  private loadMyRegistrations(): void {
    this.eventService.getMyRegisteredEvents().subscribe({
      ...
      error: (error) => console.error('Error loading my registrations:', error)
    });
  }
} 